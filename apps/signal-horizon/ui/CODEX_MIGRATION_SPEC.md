...
- Legacy `src/lib/chartTheme.ts` is removed; use `@/ui` chart defaults + helpers instead (`axisDefaults`, `xAxisNoLine`, `gridDefaults`, `gridDefaultsSoft`, `tooltipDefaults`, `legendDefaults`, `formatValue`, `alpha`, `lighten`, `darken`)
