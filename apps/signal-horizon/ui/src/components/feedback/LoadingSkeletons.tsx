...
export function StatCardSkeleton({ className = '' }: SkeletonProps) {
  return (
    <div
      className={`rounded-lg border border-gray-700 bg-gray-800/50 p-4 ${className}`}
      aria-busy="true"
      aria-label="Loading stat card"
    >
      <div className="mb-3 h-4 w-24 animate-pulse rounded bg-gray-700" />
      <div className="mb-2 h-8 w-20 animate-pulse rounded bg-gray-700" />
    ...
    >
      <div className="flex gap-4 border-b border-gray-700 bg-gray-800/70 px-4 py-3">
        {Array.from({ length: columns }).map((_, i) => (
          <div key={i} className="h-4 animate-pulse rounded bg-gray-600" style={{ width: `${60 + Math.random() * 40}px` }} />
        ))}
      </div>
      {Array.from({ length: rows }).map((_, rowIndex) => (
        <div key={rowIndex} className="flex gap-4 border-b border-gray-700/50 px-4 py-3 last:border-b-0">
          {Array.from({ length: columns }).map((_, colIndex) => (
            <div key={colIndex} className="h-4 animate-pulse rounded bg-gray-700" style={{ width: `${50 + Math.random() * 50}px` }} />
          ))}
        </div>
      ))}
    </div>
...
export function ChartSkeleton({ className = '' }: SkeletonProps) {
  return (
    ...
    >
      <div className="mb-4 h-5 w-32 animate-pulse rounded bg-gray-700" />
      <div className="relative h-48 w-full animate-pulse rounded bg-gray-700/50">
        <div className="absolute bottom-4 left-4 right-4 flex items-end justify-around gap-2">
          {Array.from({ length: 7 }).map((_, i) => (
            <div key={i} className="w-8 animate-pulse rounded-t bg-gray-600" style={{ height: `${20 + Math.random() * 80}px` }} />
          ))}
        </div>
      </div>
    </div>
